...
def extract_data(image_list, label_list,mode):
    features = []
    label_ids = []
    ...
    save_path = os.path.join(workspace_path, f"{mode}_sample_v{exp}")
    ...
    for l in label_names:
        ...
        for label_a_image_name in label_images:
            ...
            if ext in ["jpg", "png", "jpeg", "JPG", "PNG", "JPEG"]:
                ...
                print(f"file: {label_a_image_name} is not image file")
    #shuffle image_list and label_list
    train_image_list, val_image_list, train_labels, val_labels = train_test_split(image_list, label_list, test_size=0.3,
                                                                                  random_state=seed, shuffle=True)
    uni, cnts = np.unique(val_labels, return_counts=True)
    print(uni)
    print(cnts)
    #save val image list
    with open(f"./val_listv{exp}.txt", 'w') as f:
        for i in range(len(val_image_list)):
            f.write(val_image_list[i] + " " + val_labels[i] + "\n")

    extract_data(train_image_list, train_labels, 'train')
    extract_data(val_image_list, val_labels, 'eval')